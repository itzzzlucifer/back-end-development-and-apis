...
assert.isTrue(
  __listening,
  "Server is not running on port 3001. Start the server with npm start.",
);
```
...
assert.equal(
  __joinMsg.type,
  "system",
  'Join message should have type "system"',
);
assert.include(
  __joinMsg.text,
  "Alice",
  "Join message text should include the username",
);
```
...
const [__aliceGot, __bobGot] = await Promise.all([
  ...
  Promise.resolve(
    __alice.send(
      JSON.stringify({ username: "Alice", text: "Hello everyone!" }),
    ),
  ),
]);
...
assert.equal(
  __aliceGot.type,
  "chat",
  'Sender should receive their own message with type "chat"',
);
assert.equal(
  __aliceGot.text,
  "Hello everyone!",
  "Sender should receive the correct message text",
);
assert.equal(
  __bobGot.type,
  "chat",
  'Other clients should receive the message with type "chat"',
);
assert.equal(
  __bobGot.text,
  "Hello everyone!",
  "Other clients should receive the correct message text",
);
```
...
assert.equal(
  __leaveMsg.type,
  "system",
  'Leave message should have type "system"',
);
assert.include(
  __leaveMsg.text,
  "Alice",
  "Leave message text should include the disconnected username",
);
```
